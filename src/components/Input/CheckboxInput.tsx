...
export function CheckboxInput({ onBlur, onChange, isEditable = true, label, value, testID }: CheckboxInputProps) {
  ...
  const checkBoxStyles = useThemeStyles<ViewStyle>(
    ({ palette, radii, spacing }) => ({
      backgroundColor: !isEditable ? undefined : value ? palette.complimentBase : undefined,
      borderColor: !isEditable ? palette["neutralBase-40"] : value ? palette.complimentBase : palette["neutralBase-20"],
