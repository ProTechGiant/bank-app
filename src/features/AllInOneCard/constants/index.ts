//AIO genric constants
export const NERA_PLUS_CARD = 1;
export const VAT_PERCENTAGE = 15;
export const MAXIMUM_CALL_VERIFICATION_LIMIT = 4;
export const FREE_WALLET_LIMIT_FOR_NERA = 4;
export const FREE_WALLET_LIMIT_FOR_NERA_PLUS = 6;
export const DIFFERENCE_BETWEEN_NERA_PLUS_AND_NERA = 2;
export const PINCODE_LENGTH = 4;
export const NERA_CATEGORY_ID = "AIO_Nera";
export const NERA_PLUS_CATEGORY_ID = "AIO_Nera_Plus";
export const PAYMENT_METHOD_ANNUAL = "AIOAnnual";
export const ALL_BENEFITS = "all";
export const ACTIVE_BENEFITS = "activeBenefits";
export const STATUS_ACTIVE = "Activated";
export const STATUS_PENDING_ACTIVATION = "Pending_Activation";
export const STATUS_PENDING_Termination = "Pending_Termination";
export const STATUS_TERMINATED = "Terminated";
export const STATUS_INACTIVE = "Inactive";
export const HAVE_SUBSCRIPTION = false;
export const ANGHAMI_CODE = "ANGHAMI";
export const SHAHID_CODE = "SHAHID";
export const MONTHLY_SUBSCRIPTION = "AIOMonthly";
export const YEARLY_SUBSCRIPTION = "AIOAnnual";
export const EXISTING_ACTIVE_SUBSCRIPTION = "UK.AIO.CARD.SHAHID.CUSTOMER_NOT_ELIGIBLE_ERROR";
export enum AIOtype {
  Nera = "nera",
  NeraPlus = "neraPlus",
  NERA_PLUS = "nera Plus",
}
export const FREEZE = "M";
export const UNFREEZE = "_";
export const AIO_PRODUCT_TYPE = "366_SAR_001";

// Dashboard screen
export const DEFAULT_LATEST_TRANSACTIONS = "5";
export const AIO_CARD_TYPE = "EXID";

// phsical card
export const PRODUCT_CODE = "366_SAR_001";
export const FEE_TYPE = "Print";
export const NO_OF_ITEMS = "1";
export const ISSUANCE_REASON_TYPE = "Issue Physical card";
export const ACTION_TYPE = "ReissAsNew";
export const CARD_ID_TYPE_EXID = "EXID";
export const REPLACEMENT_REASON_TYPE = "Card Replacement";
export const REPLACEMENT_REASON_CODE = "ReplacementReasonCode";
export const OTP_REASON = "Card Replacement";
export const CLOSURE_REASON_CODE = "test";

// error code
export const ERROR_CODE_OTP_LIMIT_REACHED = "0034";
export const PENDING_TRANSACTIONS_REJECTION_REASON = "UK.AIO.CARD.FAILED_TO_CLOSE_CARD_TRANSACTION_ERROR";
export const REMAINING_BALANCE_REJECTION_REASON = "UK.AIO.CARD.FAILED_TO_CLOSE_CARD_BALANCE_ERROR";
export const FEES_NOT_PAID_REJECTION_REASON = "UK.AIO.CARD.FAILED_TO_CLOSE_CARD_PAYMENTS_ERROR";
export const PRODUCT_CODE_FOR_NERA = "366_SAR_001";
export const PRODUCT_CODE_FOR_NERA_PLUS = "366_SAR_002";
export const ERROR_INSUFFICIENT_BALANCE = "UK.AIO.CARD.INSUFFICIENT_BALANCE_ERROR";
